...
Do not introduce accounts, a database, a backend, a frontend framework, or a new dependency unless the maintainer explicitly approves that architectural change. The approved exception is the credential-free engagement feature under `worker/`: a narrowly scoped Cloudflare Worker and D1 database may store anonymous aggregate plugin detail views, successful command-copy actions, and hearts guarded by local browser storage. Hearts are anonymous reactions, not unique or verified votes. Do not expand it into identity, profiling, comments, scored ratings, installation telemetry, or general analytics without separate approval. Approved on 2026-09-23: the site may derive client-side orderings and selections from these anonymous aggregates, such as the install rate (install-command copies per detail view, ranked by the lower bound of the 95 % Wilson score interval) and a hidden-gems selection on the landing page; this collects no new data.
...
Root community plugins default to the generated standard Omarchy install command. Current Omarchy install and update commands obtain mutable upstream HEAD and do not accept an exact marketplace SHA, so UI copy must identify them as current-upstream and not verification-bound. Never imply that the command installs the verified snapshot. Use a manual installation override only when that command cannot produce a functioning plugin, such as when a required native binary is absent or a mandatory build must run before enablement. Optional feature setup, credentials, API configuration, data sources, bar placement, and layout customization do not make an otherwise functioning plugin manual. A curated `plugins.<id>.installation` override in `registry.json` must contain exactly `mode: "manual"` and a non-empty user-facing `note`. Manual overrides must not publish arbitrary installer commands, apply only to root plugin repositories, and must keep installation disabled and the curated note intact across passed, failed, and unreachable upstream checks. During automated listing review, finalize all setup labels, including adding or removing `manual-setup`, before setting `approved-and-verified` last; approval maps that maintainer decision to the registry override. Initial listing and update approval must resolve the immutable webhook `issue.updated_at` to exactly one approval-label transition and require that transition to remain current. Only initial resolution may accept the event in the exact webhook UTC second or exactly one second earlier, matching observed GitHub timestamp behavior; every `approved-and-verified` transition in those two seconds counts toward ambiguity. Missing or multiple transitions, events after the webhook timestamp, and events at least two seconds earlier fail closed. Later checks retain the exact selected event ID, actor, and actual event timestamp with no tolerance, while requiring that event to remain the sole transition in the original two-second compatibility window. Failed publication runs must not remove approval labels or overwrite another run's status comment. They append explicitly historical, run-scoped reports instead; a maintainer must review current evidence before manually removing and reapplying the approval label. In the write-token job, prepare the tested commit and verify the expected base before the final mutable issue, approval, baseline, permission, and upstream checks; keep those checks and the guarded push in one shell step to minimize the unavoidable cross-system race window. `approved-for-listing` is a legacy audit label and must not publish new submissions.

Explorer growth uses Git committer timestamps normalized to UTC for completed daily catalog snapshots. The current catalog's `generatedAt` sets the logical day horizon: standalone or no-op rebuilds must exclude later Git days until a newer catalog is supplied, rather than advancing or inventing a catalog timestamp. Only the last point of the previously committed series is provisional: same-day builds may update it, and the first successful later-day build may finalize it from complete Git history while retaining its previous UTC date. All older points remain field-for-field immutable. Recompute daily deltas from consecutive totals, fill missing days, and reject shallow or truncated history before replacing Explorer output. The growth view does not show a provisional-day notice; the maintainer removed it on 2026-09-23. Cover builds generated before midnight but committed after midnight with deterministic `GIT_COMMITTER_DATE` fixtures; do not manually patch generated history.
...
GitHub API rate-limit exhaustion and forbidden API access are fatal build errors; they must not degrade sources to `repository-unreachable` or produce a catalog artifact. Submission validation consumes pre-provisioned labels and must not rewrite repository label metadata on every run; use the maintainer-only `provision-labels.yml` workflow to create or repair the required labels. Pure submission and plugin-update analysis is serialized per issue with `queue: max`. After successful analysis, a strict live issue lookup may route ordinary feedback to `issue-validation-<number>`; analysis failure, an API or schema error, conflicting type labels, or any approval-sensitive label must route to `plugin-catalog-writes`. The selected group expression must clamp to exactly those two forms. Every issue write must recheck the exact live title, body, state, type labels, and approval-sensitive labels. If trust state appears while a per-issue job is queued or mutating, stop further writes and replay the same checksummed artifact through the globally locked fallback; failure compensation runs only there. Actual catalog publication, verification, delisting, refresh, and approval-sensitive validation remain in `plugin-catalog-writes`. Every admitted analysis generation is retained, and stale generations must not mutate current issue state. Before applying `approved-and-verified`, run `npm run check:catalog-write-admission` with a bounded token and exact `GITHUB_REPOSITORY`; the check fails closed unless the live group depth is at most 10, treats only an exact endpoint `404` as idle, and approvals stop entirely at 50 members. Add at most 5–10 approvals per admitted batch and use live depth rather than a fixed timer. Refresh failures are reconciled into one bot-authored operational issue with no labels and only bounded workflow metadata. `route-issue-automation.yml` is the only workflow with a direct `issues` trigger. It selects one reusable issue workflow for supported events, but it is not an authorization boundary: every called workflow must independently preserve its existing event, actor, issue-state, evidence, replay, and permission checks.
